...
impl Payload {
    pub(crate) fn build(&self, next_payload: PayloadType) -> Vec<u8> {
        ...
        match self {
            Payload::SecurityAssociation(v) => v.build(next_payload),
            Payload::KeyExchange(v) => v.build(next_payload),
            Payload::Notify(v) => v.build(next_payload),
            Payload::Delete(v) => v.build(next_payload),
            Payload::Nonce(v) | Payload::VendorID(v) | Payload::EncryptedAndAuthenticated(v) => {
                self.build_generic(next_payload, v)
            }
        }
    }
